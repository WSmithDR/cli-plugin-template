#!/bin/bash
# Tests del Stop hook detect-pending-feedback. Data dir aislado.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CPT="$SCRIPT_DIR/cpt"
HOOK="$SCRIPT_DIR/hooks/detect-pending-feedback.sh"
PASS=0; FAIL=0
...
echo ""
echo "=== detect-pending-feedback.sh ==="

# sin pendientes → sin output, exit 0
out=$(echo '{"transcript_path":""}' | bash "$HOOK" 2>&1); rc=$?
...
x
EOF
out=$(echo '{"transcript_path":""}' | bash "$HOOK" 2>&1)
